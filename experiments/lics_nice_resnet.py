...
# Create the parser
parser = argparse.ArgumentParser(description="args for nice-resnet baseline")
...
parser.add_argument("--cov_feature_dim", type=int, help="dimensionality of extracted features", default=5)
...
parser.add_argument("--treatment_feature_dim", type=int, help="dimensionality of extracted features", default=5)
...
treatment_feature_config.backbone = 'ResNet50'
...
treatment_feature_model = 'ResNet'
img_ite = ImgITE(
    covarites_feature_config,
    treatment_feature_config,
    outcome_model_config,
    treatment_feature_model=treatment_feature_model,
    reg='MMD', reg_coeff=args.reg_coeff, learning_rate = args.learning_rate
)

logger = CSVLogger("LICS_NICE" + str(args.seed), name="NICE_ResNet" + str(args.seed)) 
...
csv_file = 'metrics/NICE_metric.csv'
